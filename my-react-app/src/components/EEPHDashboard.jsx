...
export default function EEPHDashboard({
  ...
  const approve = (subId) => {
    setForwardedSubmissions((prev) =>
      prev.map((f) =>
        f.id === subId ? { ...f, status: "EEPH Approved" } : f
      )
    );
  ...
  const reject = (subId) => {
    setForwardedSubmissions((prev) =>
      prev.map((f) =>
        f.id === subId ? { ...f, status: "EEPH Rejected" } : f
      )
    );
  ...
  const forwardApprovedToDept = () => {
    ...
    setForwardSuccess("Forwarded to SEPH department successfully!");
    setTimeout(() => {
      setForwardSuccess("");
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="bg-white p-6 rounded-xl shadow border mt-6">
          <div className="flex justify-between mb-4">
            <h2 className="font-semibold text-gray-700">EEPH Dashboard</h2>
            <button
              onClick={() => {
                logout?.();
                window.location.href = "/";
